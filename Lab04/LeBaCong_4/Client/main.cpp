...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        if (strcmp(sentBuf,"FILE1") == 0) {
            strcpy(sentBuf, "FILE1");  
            write(connSock, sentBuf, strlen(sentBuf));
            FILE * fd = fopen("data.txt", "w+a");;
            while ((recvDataLen = read(connSock, recvBuf, sizeof(recvBuf))) > 1) {
                recvBuf[recvDataLen] = '\0';
                if (recvBuf[0] == '@') break;
                printf("Message received from server: %s\n", recvBuf);
                fwrite(recvBuf, sizeof(char), sizeof(recvBuf), fd);
            }
            fclose(fd);
            printf("Save file OK! \n");
        }
